    ...
    */
	...
	public function verify_paylot_transaction() {
		...
		if ( isset( $_REQUEST['paylot_txnref'] )  ){
			$paylot_url = 'https://api.paylot.co/transactions/verify/' . sanitize_text_field($_REQUEST['paylot_txnref']);
	        ...
	        if ( ! is_wp_error( $request ) && 200 == wp_remote_retrieve_response_code( $request ) ) {
				...
				if ( true == $paylot_response->sent ) {
					...
					$order_details 	= explode( '_', sanitize_text_field($_REQUEST['paylot_txnref']) );
